...
// Format number fwith decimal places
...
export function formatError(error: any) {
  if (error.name === 'ZodError') {
    ...
    return `${field.charAt(0).toUpperCase() + field.substring(1)} already exists`;
  } else {
    // Handle other errors
...
export function formUrlQuery({
  ...
  // Set key of obejct query to value from props(pageValue +1 / -1)
  query[key] = value;
