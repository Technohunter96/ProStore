...
export async function getOrderSummary() {
  ...
  // Get monthly sales - make array from raw sql query, so we can later on map through it and convert total sales decimal to number
  const salesDataRaw = await prisma.$queryRaw<
    Array<{ month: string; totalSales: Prisma.Decimal }>
  >`SELECT to_char("createdAt", 'MM/YY') as "month", sum("totalPrice") as "totalSales" FROM "Order" GROUP BY to_char("createdAt", 'MM/YY')`;

  const salesData: SalesDataType = salesDataRaw.map(({ month, totalSales }) => ({
    month,
...
// Update COD (Cash On Delivery) to paid
...
// Update COD (Cash On Delivery) order to delivered
