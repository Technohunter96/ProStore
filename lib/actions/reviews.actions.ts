...
export async function createUpdateReview(data: z.infer<typeof insertReviewSchema>) {
  try {
    ...
    // If user already reviewed, update the review, otherwise create a new one
    // Using a transaction to ensure atomicity, because if any operation fails, we want to rollback all changes
